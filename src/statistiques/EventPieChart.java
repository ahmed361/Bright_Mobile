...
public class EventPieChart extends AbstractDemoChart {
    ...
    protected CategorySeries buildCategoryDataset(String title, String[] values) {
        ...
        int k = 0;
        for (Membre c : ServiceMembre.getInstance().getlistnew()) {

          //  series.add(c.getVille(),c.getVille().compareTo(title));
           series.add(c.getVille(),c.getVille().compareTo(title));
     ...
     */
    //heli l emplacement te l projet
    public Form execute() {
        String[] values = new String[5];
        int[] colors = new int[30];
        Integer i = 0;
        for (Membre c : ServiceMembre.getInstance().getlistnew()) {
            values[i] = (String) c.getVille();
            i++;
        }
        Integer j = 0;
        for (Membre c : ServiceMembre.getInstance().getlistnew()) {
        ...
        return wrap("Statistiques Tel", comp);
